...
class ArcballControl {
	...
	triggerInitialSpin() {
		// Create a strong initial rotation impulse
		// Rotate around Y axis (0, 1, 0) relative to screen seems best for "globe spin"

		// We set a value to pointerRotation directly to simulate a "fling" frame
		// axis: Y-axis [0, 1, 0]
		// angle: large enough to cause spin
		const axis = vec3.normalize(vec3.create(), [0.1, 1, 0]); // Slight tilt
		const angle = 0.5; // Impulse strength
...
class InfiniteGridMenu {
	...
	triggerInitialSpin() {
		...
		// Physics Impulse (Spin)
		const axis = vec3.normalize(vec3.create(), [0.1, 1, 0]);
		const angle = 0.8; // Stronger impulse for dramatic effect during zoom out
		quat.setAxisAngle(this.control.pointerRotation, axis, angle);
	}

	_onControlUpdate(deltaTime) {
		const timeScale = deltaTime / this.TARGET_FRAME_DURATION + 0.0001;
		let damping = 5 / timeScale;
		let cameraTargetZ = 3; // Default close-up position

		// --- Intro Sequence Logic ---
		if (this.introActive) {
			const now = performance.now();
			const elapsed = (now - this.introStartTime) / 1000; // Seconds

			if (this.control.isPointerDown) {
				// Cancel intro if user interacts
				...
				const TOTAL_DURATION = ZOOM_OUT_DURATION + SPIN_DURATION + ZOOM_IN_DURATION;

				if (elapsed < ZOOM_OUT_DURATION) {
					// Phase 1: Zoom Out (3 -> 14)
					const t = elapsed / ZOOM_OUT_DURATION;
					// Ease out cubic
					const ease = 1 - Math.pow(1 - t, 3);
					cameraTargetZ = 3 + (14 - 3) * ease;
				} else if (elapsed < ZOOM_OUT_DURATION + SPIN_DURATION) {
					// Phase 2: Hold Zoom (14)
					cameraTargetZ = 14;
				} else if (elapsed < TOTAL_DURATION) {
					// Phase 3: Zoom In (14 -> 3)
					const t = (elapsed - (ZOOM_OUT_DURATION + SPIN_DURATION)) / ZOOM_IN_DURATION;
					// Ease in-out quadratic
					const ease = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
					cameraTargetZ = 14 + (3 - 14) * ease;
				} else {
					// End Sequence
					this.introActive = false;
				}

				// Override damping for crisp camera movement during intro
				damping = 2;
			}
		}

		const isMoving = this.control.isPointerDown || Math.abs(this.smoothRotationVelocity) > 0.01 || this.introActive;
		...
		if (!this.control.isPointerDown && !this.introActive) {
			...
			cameraTargetZ += this.control.rotationVelocity * 80 + 2.5; // Dynamic zoom on spin
			damping = 7 / timeScale;
		}

		// Apply Camera Z (Interpolation)
		// If intro is active, we want tighter control, so we used lower damping above
...
const InfiniteMenu = ({ items = [] }) => {
	...
	useEffect(() => {
		...
		if (canvas) {
			...
			sketch = new InfiniteGridMenu(canvas, items, handleActiveItem, setIsMoving, sk => {
				sk.run();
				// Trigger the magical auto-spin
				setTimeout(() => {
					sk.triggerInitialSpin();
				}, 100);
			});
		}
	...
	return (
		<div style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden' }}>
			{/* Magic Particles */}
			<div className="magic-particles-container">
				{[...Array(15)].map((_, i) => (
					<motion.div
						key={i}
						className={`magic-particle ${i % 3 === 0 ? 'purple' : ''}`}
						initial={{
...
const Projects = () => {
	...
	// Map existing INFO.projects to the format expected by InfiniteMenu
	const formattedProjects = INFO.projects.map(p => ({
		image: p.logo,      // Map logo to image
		link: p.link,
		title: p.title,
		description: p.description
	}));

	// Ensure we have enough items for the WebGL instances?
	// The WebGL code replicates items based on gl_InstanceID, so mapping directly is fine.
	...
	return (
		<React.Fragment>
			...
			<div className="page-content projects-page-wrapper">
				...
				<div className="projects-webgl-container">
					<InfiniteMenu items={formattedProjects} />
				</div>
			</div>
		</React.Fragment>
	);
